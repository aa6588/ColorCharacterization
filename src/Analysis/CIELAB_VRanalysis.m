...
%% Divide each illuminant's results_b_
cd ..\..\
load Flat_model.mat 
...
illum_uvY = XYZ2Lab(xyY2XYZ(illum_xyY),whitepoint('D65').*100);
...
rawTable = finalTable;
 finalTable.uvY = XYZ2Lab(finalTable.XYZ,whitepoint("d65").*Flat_model.w.wp(2));
%% calculate CIs
cd C:\Users\Andrea\Documents\GitHub\ColorCharacterization\src\Analysis\
...
avgTable.GroupCount = [];
avgTable_uvY = groupsummary(white_table,{'ParticipantID','Illuminant'},'mean','uvY');
...
for i = 1:length(illuminants)
    ...
    adj_uv_white = avgTable_uvY.mean_uvY;
    ...
    for part = 1:length(obsXYZ)
        ...
        recentered_uv = XYZ2Lab(xyY2XYZ(uvY2xyY(recentered_uv)),D65XYZ);
        [CI_value, delta_uv_value] = computeCIproj(w_uv(2:3), chrom_test_uv(2:3), w_uv(2:3), recentered_uv(2:3));
        finalTable.recenter_uv_2(rows(part), :) = recentered_uv(2:3); 
        finalTable.CI_2_recenter(rows(part)) = CI_value;
        finalTable.delta_uv_2_recenter(rows(part)) = delta_uv_value;
        [CI_value, delta_uv_value] = computeCIproj(w_uv(2:3), chrom_test_uv(2:3), adj_uv_white(part,2:3), adj_uv(part,2:3));
...
save('FlatDataCIELAB.mat','rawTable','finalTable');