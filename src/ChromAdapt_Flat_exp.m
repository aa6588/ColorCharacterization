function [participant] = ChromAdapt_Flat_exp(participant,paths)

%% Main function for Color Constancy Experiment Flat
% Create the connection
t = tcpip('127.0.0.1', 8890);
fopen(t);

%load variables
load Flat_RGB_grids_struct.mat Flat_RGB_grid
load Flat_LAB_grid_struct.mat Flat_LAB_grid
%load models_info.mat model
load Flat_illums.mat flatRGB_illum 
%% Define randomization of vars

%lights
RGB_white_illum = flatRGB_illum(1,:);
RGB_chrom_illum = flatRGB_illum(2:end,:);
illum_color = {'r','g','b','y'};
...
for rep = 1:3 %repeat exp 3 times
    ...
    for L = 1:length(L_star)
        [curr_select,curr_lab_select,curr_lab_start,Idx] = run_experiment('w',RGB_white_illum,rand_L{L},Flat_RGB_grid,Flat_LAB_grid,t);
...
adapting(RGB_chrom_illum(illum_rand(illums),:),t,120); %adapt to illum for 2 mins
    for rep = 1:3
        ...
        for L = 1:length(L_star)
            ...
            [curr_select,curr_lab_select,curr_lab_start,Idx] = run_experiment(curr_illum,curr_illumRGB,rand_L{L},Flat_RGB_grid,Flat_LAB_grid,t);
            tempTable = table(curr_select,curr_lab_select,curr_lab_start,Idx,'VariableNames',colnames);
...
%add interjection of white illum trial L55 only
    if illums < 4
        ...
        [curr_select,curr_lab_select,curr_lab_start,Idx] = run_experiment('w',RGB_white_illum,'L55',Flat_RGB_grid,Flat_LAB_grid,t);
        tempTable = table(curr_select,curr_lab_select,curr_lab_start,Idx,'VariableNames',colnames);
